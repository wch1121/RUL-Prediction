...
class DampingLayer(nn.Module):
    ...
    def __init__(self, pred_len, nhead, dropout=0.1, output_attention=False):
        ...
        initial_tensor = torch.randn(pred_len, nhead)
        self.ouput_param = nn.Parameter(initial_tensor)
...
class Decoder(nn.Module):
    ...
    def __init__(self, layers):
        ...
        self.pred_growth = nn.Linear(self.d_model, self.c_out)
        self.pred_level = nn.Linear(self.d_model, self.c_out)

    def forward(self, growths):
        growth_repr = []
        level_repr = []
        growth_dampings = []

        for idx, layer in enumerate(self.layers):
            growth_horizon, growth_damping = layer(growths[idx])
            growth_repr.append(growth_horizon)
            growth_dampings.append(growth_damping)
        growth_repr = sum(growth_repr)
        growth_repr = self.pred_growth(growth_repr)
        return growth_repr, growth_dampings